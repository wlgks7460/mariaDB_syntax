...
DESCRIBE author;
-- post 테이블 신규 생성
-- 외래키가 설정되면 post 테이블 데이터의 생성, 삭제, 수정에대해 제약이 발생
-- 만약 not null 조건이 있다면 auhtor_id가 데이터는 post에 생성 불가
-- author가 상제 될때 post에 대이터가 남아있으면 author tkrwp qnfrk
...
-- 테이블 index 조회
-- 제약 조건 정보 조회와 인덱스조회는 상당수 일치(pf,fk 제약조건은 인덱스를 생성)
